...
for /l %%i in (1, 1, %length%) do (
    ...
    for /f "delims=" %%a in ('powershell -NoProfile -Command "if (!max_local_time!) { [DateTime]::new(!max_local_time!, 'UTC').ToString('yyyy-MM-dd HH:mm:ss UTC') } else { '无' }"') do set "max_local_time=%%a"
    for /f "delims=" %%a in ('powershell -NoProfile -Command "if (!max_backup_time!) { [DateTime]::new(!max_backup_time!, 'UTC').ToString('yyyy-MM-dd HH:mm:ss UTC') } else { '无' }"') do set "max_backup_time=%%a"
